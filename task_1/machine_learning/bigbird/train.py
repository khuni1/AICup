...
for fold, (train_idx, val_idx) in enumerate(skf.split(df["text"], df["label"]), start=1):
    ...
    data_collator = MinLenPadCollator(tokenizer, min_len=768, multiple=64)
    trainer = Trainer(
        model=model,
        args=training_args,
...
plt.title(f"OOF Confusion Matrix (BigBird raw, theta*={theta_star:.3f})")
plt.savefig(os.path.join(OUT_DIR, "OOF_confusion_matrix_BigBird_raw.png"), dpi=150, bbox_inches="tight")
plt.close()

with open(os.path.join(OUT_DIR, "OOF_metrics_BigBird_raw.json"), "w", encoding="utf-8") as f:
    json.dump({
...
disp.plot(cmap=plt.cm.Blues)
plt.title(f"OOF Confusion Matrix (BigBird calibrated: {CAL_METHOD}, theta=0.5)")
plt.savefig(os.path.join(OUT_DIR, f"OOF_confusion_matrix_BigBird_{CAL_METHOD}_theta0.5.png"),
            dpi=150, bbox_inches="tight")
plt.close()

with open(os.path.join(OUT_DIR, f"OOF_metrics_BigBird_{CAL_METHOD}_theta0.5.json"), "w", encoding="utf-8") as f:
    json.dump({
