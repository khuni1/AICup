...
# This is the best embedding file you found for the GAT:
EMBEDDING_FILE = 'node_embeddings_p1.2_q1.0.kv' 
# FINAL MODEL WEIGHTS WILL BE SAVED HERE
MODEL_SAVE_PATH = 'ensemble_model_B_fixed_gat.pth' 
LEARNING_RATE = 0.001
EPOCHS = 300 

# --- 1. Data Processing for GNN ---
def parse_and_create_pyg_data_with_embeddings(data_file, labels_map, keyed_vectors, y_labels=None):
    """Parses FCG data and creates PyG Data objects with Fixed Embeddings + Degrees."""
    graphs_list = []
    
    with open(data_file, 'r') as f:
        lines = f.readlines()
        for i, line in enumerate(lines):
            # (Data parsing logic is correctly retained from your original script)
            ...
            for node_name in unique_nodes:
                try:
                    ...
                    # Use zero vector for nodes not in embedding file
...
class GAT(torch.nn.Module):
    def __init__(self, num_node_features, num_classes, heads=4, dropout=0.6):
        super(GAT, self).__init__()
        # Using the best GAT hyperparameters from your grid search
        self.conv1 = GATConv(num_node_features, 64, heads=heads, dropout=dropout)
...
def train_and_save_final_model(all_train_graphs, num_node_features, num_classes):
    ...
    model = GAT(num_node_features=num_node_features, num_classes=num_classes, heads=4, dropout=0.6)
    ...
    print(f"Training final Model B (Fixed Embeddings GAT) on ALL {len(all_train_graphs)} samples for {EPOCHS} epochs...")
    ...
    print(f"\nModel B weights saved successfully to: {MODEL_SAVE_PATH}")


# --- Main Execution ---
if __name__ == '__main__':
    print("--- Preparing Model B (Fixed Embeddings GAT) for Ensemble ---")
